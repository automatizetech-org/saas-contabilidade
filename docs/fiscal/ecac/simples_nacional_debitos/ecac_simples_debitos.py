...
class DashboardClient:
    ...
    def derive_segment_path_from_folder_structure(self, payload: Optional[dict[str, Any]]) -> str:
        ...
        elif self.technical_id == "ecac_simples_debitos":
            candidates = {
                "debitos do simples nacional",
                "consulta debitos",
                "consulta de debitos",
                "simples nacional debitos",
            }
        else:
...
DEBITOS_MENU_LABELS = ["Debitos", "Débitos"]
DEBITOS_CONSULTAR_LABELS = ["Consultar/Gerar DAS", "Consultar / Gerar DAS", "Consultar Gerar DAS"]
...
def _parse_currency(value: str) -> str:
    match = re.search(r"\d{1,3}(?:\.\d{3})*,\d{2}", str(value or ""))
    return match.group(0) if match else ""


def collect_debito_rows(frame: Any) -> list[dict[str, Any]]:
    try:
        rows = frame.evaluate(
            """() => {
                const normalize = (value) => String(value || '')
    ...
    for item in rows or []:
        if not isinstance(item, dict):
            continue
        periodo = str(item.get("periodo_apuracao") or "").strip()
        ...
        normalized.append(
            {
                ...
                "debito_declarado": _parse_currency(str(item.get("debito_declarado") or "")),
                "principal": _parse_currency(str(item.get("principal") or "")),
...
def process_company(automation: SimplesEcacAutomation, company: CompanyRecord, company_dir: Path, args: argparse.Namespace) -> CompanyExecutionResult:
    ...
    automation.runtime.logger.info(
        f"{company.name} [{format_document(company.document)}] | Abrindo PGDAS-D para consultar debitos."
    )
    ...
    if frame is None:
        raise RuntimeError("Frame do PGDAS-D nao ficou disponivel para consulta de debitos.")

    result.flags["debitos_menu_opened"] = automation.click_anywhere(DEBITOS_MENU_LABELS, timeout_ms=12000)
    try:
        result.flags["debitos_consulta_opened"] = bool(
            frame.evaluate(
                """labels => {
                    const normalize = (value) => String(value || '')
                        .normalize('NFD')
                ...
                DEBITOS_CONSULTAR_LABELS,
            )
        )
    except Exception:
        result.flags["debitos_consulta_opened"] = False
    if not result.flags["debitos_consulta_opened"]:
        result.flags["debitos_consulta_opened"] = automation.click_anywhere(DEBITOS_CONSULTAR_LABELS, timeout_ms=12000)
    ready = automation._wait_until(
        lambda: "discriminacao dos debitos do simples nacional" in _normalize_portal_text(automation.collect_scope_text(frame))
        or (
            "numero parcelamento" in _normalize_portal_text(automation.collect_scope_text(frame))
            and "periodo de apuracao" in _normalize_portal_text(automation.collect_scope_text(frame))
        ),
        timeout_ms=30000,
        interval_ms=400,
    )
...
class SimplesWorker(QThread):  # type: ignore[misc]
    ...
    def run(self) -> None:
        try:
            self.status.emit("Executando")
            runner = RobotRunner(technical_id=ROBOT_TECHNICAL_ID, display_name=ROBOT_DISPLAY_NAME, args=self.args, runtime_dir=None, process_company=process_company)
            self.runner = runner
...
class MainWindow(QMainWindow):
    ...
    def _on_job_poll(self) -> None:
        ...
        if not pending_execution_request_id or pending_execution_request_id == self._last_job_execution_request_id:
            return
...
def _run_cli(args: argparse.Namespace) -> int:
    runner = RobotRunner(technical_id=ROBOT_TECHNICAL_ID, display_name=ROBOT_DISPLAY_NAME, args=args, runtime_dir=None, process_company=process_company)
