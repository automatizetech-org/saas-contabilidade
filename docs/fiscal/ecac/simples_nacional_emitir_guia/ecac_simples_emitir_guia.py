...
def build_effective_execution_args(args: argparse.Namespace, job: Optional[JobPayload]) -> argparse.Namespace:
    ...
    competencia = _normalize_job_competencia(raw.get("competence") or raw.get("competencia"))
    data_vencimento = _normalize_job_due_date(
        raw.get("recalculate_due_date") or raw.get("data_vencimento") or raw.get("due_date")
    )
            ...
            def _process_company_with_automation(
                ...
                *,
                ...
                if not company.eligible:
                    blocked = _blocked_result(company)
                    return blocked
                try:
                    if switch_profile:
...
VENCIMENTO_SELECTORS = [
    "input[name*='venc' i]",
    "input[id*='venc' i]",
    "input[placeholder*='venc' i]",
    "input[aria-label*='venc' i]",
]
CONFIRM_LABELS = ["Continuar", "Prosseguir", "Confirmar", "OK", "Gerar", "Emitir"]
DOWNLOAD_LABELS = ["Imprimir DAS", "Baixar DAS", "Download DAS", "Gerar DAS"]
...
def process_company(
    ...
    competencia = str(getattr(args, "competencia", "") or "").strip()
    data_vencimento = str(getattr(args, "data_vencimento", "") or "").strip()

    iframe_src = automation.open_pgdas_defis()
    result.flags["pgdas_opened"] = True
    result.flags["pgdas_iframe_src"] = iframe_src
    frame = automation.wait_for_pgdas_ready()
    result.flags["pgdas_ready"] = bool(automation.collect_scope_text(frame))
    result.flags["declaracao_mensal_opened"] = automation.click_anywhere(DECLARACAO_MENSAL_LABELS, timeout_ms=12000)
    result.flags["das_generation_started"] = automation.click_anywhere(GERAR_DAS_LABELS, timeout_ms=12000)

    if competencia:
        result.flags["competencia_informada"] = automation.fill_first_visible(COMPETENCIA_SELECTORS, competencia, scopes=[frame])
    else:
        result.warnings.append("Competencia nao informada; fluxo mantido apenas ate a tela segura.")
    if data_vencimento:
        result.flags["data_vencimento_informada"] = automation.fill_first_visible(VENCIMENTO_SELECTORS, data_vencimento, scopes=[frame])

    if dry_run:
        result.status = "partial"
        result.warnings.append("Parametros de emissao nao foram informados; DAS real nao foi emitido.")
        result.records.append(default_company_payload(company, {"competencia": competencia, "data_vencimento": data_vencimento}))
        return result

    for label in CONFIRM_LABELS:
        if automation.click_anywhere([label], timeout_ms=6000):
            result.flags["confirm_clicked"] = True
            break

    download_path = company_dir / f"das_{re.sub(r'[^0-9]', '', competencia or 'sem_competencia')}.pdf"
    saved = automation.expect_download_click(frame, DOWNLOAD_LABELS, download_path)
    if saved is not None and saved.exists():
        result.flags["das_generated"] = True
        result.flags["download_completed"] = True
        result.files.append({"kind": "das", "path": str(saved), "filename": saved.name})
        result.status = "success"
    else:
        result.status = "partial"
        result.warnings.append("Fluxo real foi tentado, mas nenhum download do DAS foi capturado.")
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel(ROBOT_DISPLAY_NAME)
        title.setStyleSheet("font-size: 24px; font-weight: 700; color: #14213d;")
        subtitle = QLabel("Autenticacao via certificado, troca de perfil e emissao segura de DAS do Simples.")
        ...
        self.office_label = QLabel(self.office_context.office_id or "office_id nao resolvido")
        self.job_label = QLabel("job.json nao carregado")
        for label, widget in (("Status", self.status_label), ("Progresso", self.progress_label), ("office_id", self.office_label), ("job_id", self.job_label)):
    ...
    def _load_initial_state(self) -> None:
        ...
        self.job_label.setText(self.job.job_id if self.job else "execucao manual")
    ...
    def _fallback_manual_company(self) -> list[CompanyRecord]:
        try:
            ...
            self.append_log(f"Falha ao montar contexto manual local: {exc}")
            return []
    ...
    def reload_companies(self) -> None:
        try:
            ...
            self.office_label.setText(self.office_context.office_id or "office_id nao resolvido")
    ...
    def _apply_company_filter(self) -> None:
        ...
        for company in self.companies:
            ...
            item = QListWidgetItem(f"{company.name}\n{format_document(company.document)} | {'Elegivel' if company.eligible else 'Bloqueada'}")
    ...
    def start_worker(self) -> None:
        ...
        if not self.companies:
            QMessageBox.warning(self, "Empresas", "Nenhuma empresa foi carregada.")
            return
        self.worker = SimplesWorker(self._build_runtime_args(), list(self.companies), self.job or self._build_manual_job())
    ...
    def _on_summary(self, summary: object) -> None:
        if isinstance(summary, dict):
            total = len(summary.get("company_results") or [])
            self.progress_label.setText(f"{total} / {total}")
            self.status_label.setText(str(summary.get("status") or "Concluido"))
...
def main() -> int:
    ...
    if QApplication is None:
        raise RuntimeError("PySide6 nao esta disponivel para a interface grafica.")
    app = QApplication(sys.argv)
    runtime_env = build_runtime(ROBOT_TECHNICAL_ID, ROBOT_DISPLAY_NAME, runtime_dir=SCRIPT_DIR)
