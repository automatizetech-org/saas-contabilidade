...
def process_company(automation: SimplesEcacAutomation, company: CompanyRecord, company_dir: Path, args: argparse.Namespace) -> CompanyExecutionResult:
    ...
    if not result.flags["gerar_das_opened"]:
        result.flags["gerar_das_opened"] = automation.click_anywhere(GERAR_DAS_LABELS, timeout_ms=12000)
    automation.runtime.logger.info(
    ...
    result.flags["continuar_clicked"] = automation.click_anywhere(CONTINUAR_LABELS, timeout_ms=12000)
    automation.runtime.logger.info(
    ...
    no_value_message = _extract_portal_message(body_text, SEM_VALOR_MESSAGE)
    if no_value_message:
        ...
        result.records.append(
            default_company_payload(
                ...
                {
                    "competencia": competencia,
                    "portal_message": no_value_message,
                    "data_vencimento": "",
                    "validade_calculo": "",
                    "saldo_devedor": "",
                },
    ...
    if nova_data_vencimento:
        result.flags["recalculo_opened"] = automation.click_anywhere(CONSOLIDAR_OUTRA_DATA_LABELS, timeout_ms=12000)
        ...
        result.flags["recalculo_confirmado"] = automation.click_anywhere(CONTINUAR_LABELS + ["Consolidar"], timeout_ms=12000)
