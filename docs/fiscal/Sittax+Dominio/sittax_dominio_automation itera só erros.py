...
def parse_period(period: str) -> tuple[int, int] | None:
    match = re.fullmatch(r"(\d{2})/(\d{4})", (period or "").strip())
    if not match:
        return None
...
def build_retry_queue(rows: Iterable[dict[str, str]]) -> dict[tuple[int, int], list[CompanyRow]]:
    queue: dict[tuple[int, int], list[CompanyRow]] = {}
    for row in rows:
        ...
        if period is None or not company or not cnpj:
            continue

        queue.setdefault(period, []).append(
            CompanyRow(
                cnpj=cnpj,
                company=company,
                period=row["period"],
...
def process_companies_for_period(
    page: Page,
    results: list[dict[str, str]],
    results_index: dict[tuple[str, str], dict[str, str]],
    companies: list[CompanyRow],
    month: int,
    year: int,
    *,
    ignore_retry_limit: bool = False,
) -> None:
    configured_period = configure_generate_grid(page, month, year)
    log(f"Total de empresas na fila para {configured_period}: {len(companies)}")
    ...
    while pending:
        ...
        for row_meta in pending:
            ...
            if not ignore_retry_limit and attempts >= MAX_RETRIES_PER_ENTRY:
                log(
            ...
            try:
                ...
                search_input(page).fill(row_meta.cnpj)
                page.keyboard.press("Enter")
        ...
        if not ignore_retry_limit and len(next_pending) == len(pending) and all(
...
def run() -> None:
    ...
    retry_queue = build_retry_queue(results)
    log(f"Registros concluidos no relatorio: {completed_count}")
    log_existing_progress(results)
    if retry_queue:
        retry_count = sum(len(companies) for companies in retry_queue.values())
        log(
            f"Modo de reprocessamento focado ativado: {retry_count} empresa(s) com erro "
            f"em {len(retry_queue)} competencia(s)"
        )
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            open_generate_page(page)
            if retry_queue:
                for year, month in sorted((year, month) for month, year in retry_queue):
                    companies = retry_queue[(month, year)]
                    log(
                        f"Iniciando reprocessamento da competencia {format_period(month, year)} "
                        f"com {len(companies)} empresa(s)"
                    )
                    open_generate_page(page)
                    process_companies_for_period(
                        page,
                        results,
                        results_index,
                        companies,
                ...
                for month in range(1, 13):
                    ...
                    process_companies_for_period(page, results, results_index, companies, month, TARGET_YEAR)
        finally:
            write_report(results)
