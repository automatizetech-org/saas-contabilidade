-- Lista ZIP NFE/NFC: inclui doc_type (NFE/NFC) para o front montar zip_inner_segment mesmo se modelo vier nulo no JSON.
  ...
  if v_kind = 'NFE_NFC' then
    ...
    select coalesce(
      (
        ...
        from (select file_path, empresa, modelo, doc_type from deduped limit (select limit_count from params)) s
      ),
      '[]'::jsonb
    )
    into v_payload;
