...
begin
  ...
  with params as (
    select
      coalesce(company_ids, '{}'::uuid[]) as filter_company_ids,
      coalesce(array_length(company_ids, 1), 0) as filter_company_count,
      public.normalize_search_text(search_text) as normalized_search_text,
      public.only_digits(search_text) as digits_search,
      btrim(coalesce(search_text, '')) as raw_search_text,
      least(date_from, date_to) as filter_date_from,
      greatest(date_from, date_to) as filter_date_to,
      lower(coalesce(nullif(btrim(file_kind), ''), 'all')) as filter_file_kind,
      lower(coalesce(nullif(btrim(origem_filter), ''), 'all')) as filter_origem,
      lower(coalesce(nullif(btrim(modelo_filter), ''), 'all')) as filter_modelo,
      lower(coalesce(nullif(btrim(certidao_tipo_filter), ''), 'all')) as filter_certidao_tipo,
      greatest(least(coalesce(limit_count, 25), 200), 1) as page_limit
  ),
  base as (
    select
      i.*
    ...
    where i.office_id = v_office_id
      ...
      and (p.filter_company_count = 0 or i.company_id = any (p.filter_company_ids))
      and (p.filter_date_from is null or coalesce(i.document_date, i.created_at::date) >= p.filter_date_from)
      and (p.filter_date_to is null or coalesce(i.document_date, i.created_at::date) <= p.filter_date_to)
      and (
        p.normalized_search_text = ''
        or i.search_text_normalized like '%' || p.normalized_search_text || '%'
        or (p.digits_search <> '' and public.only_digits(coalesce(i.cnpj, '')) like '%' || p.digits_search || '%')
        or coalesce(i.chave, '') like '%' || p.raw_search_text || '%'
      )
      and (
        p.filter_file_kind = 'all'
        or (p.filter_file_kind = 'xml' and i.file_extension = 'xml')
        or (p.filter_file_kind = 'pdf' and i.file_extension = 'pdf')
      )
      and (
        p.filter_origem = 'all'
        or v_kind <> 'NFS'
        or coalesce(i.origem, '') = p.filter_origem
      )
      and (
        p.filter_modelo = 'all'
        or v_kind <> 'NFE_NFC'
        or coalesce(i.modelo, '') = p.filter_modelo
      )
      and (
        p.filter_certidao_tipo = 'all'
        or v_kind <> 'CERTIDOES'
        or coalesce(i.tipo_certidao, '') = p.filter_certidao_tipo
      )
      and (
  ...
  select jsonb_build_object(
    ...
    'nextCursor',
    case
      when (select count(*) from base) > (select page_limit from params) then (
        select jsonb_build_object(
          'sortDate', sort_date,
    ...
    'hasMore', ((select count(*) from base) > (select page_limit from params)),
    'refreshAt', now()
  )
  into v_payload;
