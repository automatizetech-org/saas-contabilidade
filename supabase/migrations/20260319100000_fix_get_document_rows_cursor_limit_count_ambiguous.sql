-- Corrige ambiguidade de limit_count nas subqueries: usar alias p (params p) e p.limit_count.
...
begin
  ...
  with params as (
    select
      coalesce(get_document_rows_cursor.company_ids, '{}'::uuid[]) as company_ids,
      coalesce(array_length(get_document_rows_cursor.company_ids, 1), 0) as company_count,
      lower(coalesce(nullif(btrim(get_document_rows_cursor.category_filter), ''), 'todos')) as category_filter,
      lower(coalesce(nullif(btrim(get_document_rows_cursor.file_kind), ''), 'todos')) as file_kind,
      ...
      least(get_document_rows_cursor.date_from, get_document_rows_cursor.date_to) as date_from,
      greatest(get_document_rows_cursor.date_from, get_document_rows_cursor.date_to) as date_to,
  ...
  base as (
    ...
    where i.office_id = v_office_id
      ...
      and (
        get_document_rows_cursor.cursor_sort_date is null
        or (
          (coalesce(i.document_date, i.created_at::date), i.created_at, i.source_record_id)
          < (get_document_rows_cursor.cursor_sort_date, coalesce(get_document_rows_cursor.cursor_created_at, 'infinity'::timestamptz), coalesce(get_document_rows_cursor.cursor_id, 'ffffffff-ffff-ffff-ffff-ffffffffffff'::uuid))
        )
      )
