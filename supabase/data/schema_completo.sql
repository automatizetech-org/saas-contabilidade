...
create table if not exists public.fiscal_documents (
  ...
  office_id uuid not null default public.current_office_id() references public.offices(id) on delete cascade,
  company_id uuid not null references public.companies(id) on delete cascade,
  type text not null check (type in ('NFS', 'NFE', 'NFC', 'GUIA_SIMPLES_DAS')),
  chave text,
...
create index if not exists fiscal_documents_office_type_company_period_idx on public.fiscal_documents (office_id, type, company_id, periodo desc);
